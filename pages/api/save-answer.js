...
import User from "@/models/User";
import UserAnswer from "@/models/UserAnswer";
import mongoose from "mongoose"; // ✅ Needed to convert to ObjectId
...
export default async function handler(req, res) {
    ...
    try {
        ...
        const existingAnswer = await UserAnswer.findOne({
            user_id: user._id,
            question_id: new mongoose.Types.ObjectId(questionId), // ✅ Ensure type match
        });
        if (existingAnswer) {
            return res.status(400).json({ message: "You have already answered this question" });
        }
        ...
        const userAnswer = new UserAnswer({
            user_id: user._id,
            question_id: new mongoose.Types.ObjectId(questionId), // ✅ Ensure correct type
            answer,
        });
